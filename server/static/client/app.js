...
function renderWaitPlot(container, deliveries) {
    ...
    if (points.length < 2) {
        container.innerHTML = '<p class="text-muted" style="text-align:center">Dati insufficienti per il grafico</p>';
        return;
    }
    ...
    var data = points.map(function(d) {
        return { x: toMinutes(d.order_date), y: d.waiting_seconds / 60, _d: d };
    });

    container.innerHTML = '<canvas style="height:220px"></canvas>';
    ...
    new Chart(canvas, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(ctx) {
                            var d = ctx.raw._d;
                            return '#' + d.order_id + ' — ' + fmtWait(d.waiting_seconds);
                        }
                    }
                }
            },
            scales: {
                ...
                y: {
                    beginAtZero: true,
                    title: { display: true, text: 'Attesa (min)' },
                    ticks: { callback: function(val) { return val + 'm'; } }
                }
            }
        }
    });
...
async function screenWaitingTimes() {
    ...
    async function refreshTable() {
        try {
            ...
            renderWaitPlot($id('delivery-plot'), all);
